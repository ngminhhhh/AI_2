...
def alpha_beta_engine(chess_state, depth, maximize, alpha=float("-inf"), beta=float("inf")):
    turn = "white" if maximize else "black"

    if depth == 0:
        return (evaluate(chess_state, turn), None)
    ...
    if maximize:
        ...
        for piece in chess_state.white_pieces:
            ...
            for move in moves:
                ...
                value, _ = alpha_beta_engine(chess_state=chess_state, depth=depth-1, alpha=alpha, beta=beta, maximize=False)
        ...
        for piece in chess_state.black_pieces:
            ...
            for move in moves:
                move_info = chess_state.make_move(piece, move)
                value, _ = alpha_beta_engine(chess_state=chess_state, depth=depth-1, alpha=alpha, beta=beta, maximize=True)
